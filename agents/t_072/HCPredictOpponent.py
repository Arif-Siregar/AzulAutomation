...
class myAgent():
    def __init__(self, _id):
        ...
        self.ene_id = (self.id + 1) % 2

    def GetActions(self, state, id):
        return self.game_rule.getLegalActions(state, id)
    
    def EneSelectAction(self, rootstate, id):
        score_to_beat = 0
        new_actions = self.GetActions(rootstate, id)

        i = 0
        while (i < len(new_actions)):
            temp_old_state = deepcopy(rootstate)
            temp_new_state = self.game_rule.generateSuccessor(temp_old_state, new_actions[i], id)
            new_round_score , _ = temp_new_state.agents[id].ScoreRound()
            new_end_game_score = temp_new_state.agents[id].EndOfGameScore()
    ...
    def SelectAction(self, actions, rootstate):
        ...
        new_actions = self.GetActions(rootstate, self.id)
        best_action = None
        ...
        while (i < len(new_actions)) and (time.time()-start_time < THINKTIME):
            ...
            temp_total = new_round_score + new_end_game_score
            ene_score = self.EneSelectAction(temp_new_state, self.ene_id)
            temp_total -= ene_score
            if temp_total >= score_to_beat:
                score_to_beat = temp_total
