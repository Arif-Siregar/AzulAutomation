...
IN_MODEL_PATH = "models/DQNModelv4_4.h5"
OUT_MODEL_PATH = "models/DQNModelv4_5.h5"
...
class AdvancedGame(Game):
    ...
    def Run(self):
        ...
        while not self.game_rule.gameEnds() and action_counter <= self.max_iter_ep:
            ...
            if agent_index == 0:
                ...
                next_state, reward, done = self.step(selected, agent_index)
    ...
    def step(self, action, a_id):
        ...
        gs_copy = copy.deepcopy(self.game_rule.current_game_state)
        gs_copy_2 = copy.deepcopy(gs_copy)
        ns = self.game_rule.generateSuccessor(gs_copy,action,a_id)
        ...
        # Current game score
        curr_round_score, _ = gs_copy.agents[a_id].ScoreRound()
        curr_bonus = gs_copy.agents[a_id].EndOfGameScore()

        next_round_score, _ = ns_copy.agents[a_id].ScoreRound()
        next_bonus =  ns_copy.agents[a_id].EndOfGameScore()
        ...
        cur_p_state = gs_copy_2.agents[a_id]
        next_p_state = ns_copy_2.agents[a_id]

        # cur_o_state = gs_copy_2.agents[0]
        # next_o_state = ns_copy_2.agents[0]

        # # Preferably get tiles for the middle 3 rows + columns
        # if action[2].pattern_line_dest == 1 and action[2].tile_type in [utils.Tile.BLUE,utils.Tile.YELLOW, utils.Tile.RED]:
        #     reward+=1
        # elif action[2].pattern_line_dest == 2 and action[2].tile_type in [utils.Tile.WHITE,utils.Tile.BLUE, utils.Tile.YELLOW]:
        #     reward+=1
        # elif action[2].pattern_line_dest == 3 and action[2].tile_type in [utils.Tile.BLUE,utils.Tile.WHITE]:
        #     reward+=1
...
class myAgent():
    ...
    def SelectAction(self, actions, cur_state):
        ...
        q_values = self.model.predict(getPlayerState(cur_state.agents[self.id]))[0] 
        maxq = 0
        maxidx = 0
        max_action_int = 0
    ...
    def train(self):
        ...
        for exp in batch:
            ...
            state_feature = getPlayerState(exp["current_state"].agents[agent_id])
            ...
            if not exp["done"]:
                next_q_values = self.target_model.predict(getPlayerState(exp["next_state"].agents[agent_id]))[0]
                max_q = 0
...
class GameInfo():
    ...
    def getActionSize(self):
        ...
        size *= 5
        # Potential pattern lines to place tiles (5)
...
if __name__ == "__main__":
    ...
    for e in range(n_episodes):
        ...
        log.info(f"Win count - alec:{win_count[0]}, dqn:{win_count[1]}")
        log.info(f"Average score - alec:{avg_score[0]}, dqn:{avg_score[1]}")
